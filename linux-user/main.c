...
{
    printf("qemu-" TARGET_NAME " version " QEMU_FULL_VERSION "-" QEMU_BUILD_ARCH
	   "\n(" QEMU_GIT_VERSION ")"
           "\n" QEMU_COPYRIGHT "\n");
    exit(EXIT_SUCCESS);
}
...
//    preserve_argv0 = !!(qemu_getauxval(AT_FLAGS) & AT_FLAGS_PRESERVE_ARGV0);
     ...
     */
#if 0 //for debug
    if(argv[optind+1] && !strcmp("binfmt",argv[optind+1])) {
        optind+=2;
    }
    printf("./qemu-x86_64 %s\n", argv[optind]);
#else
    if (optind + 1 < argc && preserve_argv0  && !strcmp(argv[1], argv[2])) {
        optind++;
    }
#endif
#if defined(CONFIG_NATIVE_LIBS_CALL_DEBUG)
    // 初始化缓冲区为空  native_libs_write_path[256];
    memset(native_libs_write_path, 0, sizeof(native_libs_write_path));
    ...
    // 安全复制 /tmp/ 到缓冲区
    strncpy(native_libs_write_path, tmp_dir, sizeof(native_libs_write_path) - 1);
    size_t current_len = tmp_dir_len; // 当前已写入长度

    // 2. 提取应用名（argv[optind] 最后一个 '/' 后的内容）
    char *last_slash = strrchr(argv[optind], '/');
    const char *app_name = (last_slash != NULL) ? (last_slash + 1) : argv[optind];
    size_t app_name_len = strlen(app_name);

    // 3. 追加应用名到 /tmp/ 后（检查剩余空间）
    size_t remaining_space = sizeof(native_libs_write_path) - current_len - 1;
    if (app_name_len > remaining_space)
    {
        fprintf(stderr, "应用名过长，无法写入缓冲区\n");
        return -1; // 或处理错误
    }
    strncat(native_libs_write_path + current_len, app_name, app_name_len);
    current_len += app_name_len;

    // 4. 追加 "-PID.txt"
    char pid_suffix[20];
    int pid = getpid();
    snprintf(pid_suffix, sizeof(pid_suffix), "-%d.txt", pid);
    size_t pid_suffix_len = strlen(pid_suffix);

    // 检查最终剩余空间
    remaining_space = sizeof(native_libs_write_path) - current_len - 1;
    if (pid_suffix_len > remaining_space)
    {
        fprintf(stderr, "PID后缀过长，路径截断: %s\n", native_libs_write_path);
        return -1;
    }
    strncat(native_libs_write_path + current_len, pid_suffix, pid_suffix_len);

    // 最终路径示例: /tmp/wechat-1234.txt
    printf("调试文件路径: %s\n", native_libs_write_path);
#endif
