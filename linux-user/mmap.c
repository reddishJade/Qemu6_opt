                     ...
                     int prot, int flags, int fd, abi_ulong offset)
{
	...
	void *p;
        ...
        {
            p = mmap(host_start, qemu_host_page_size, prot,
                    flags, fd, offset);
            pflags = 1;
        }else{
            p = mmap(host_start, qemu_host_page_size, prot,
                     ...
                     int flags, int fd, abi_ulong offset)
{
    ...
    if (fd > 2)
    {
        ...
        if (libname_len != -1)
        {
            ...
            {
                *dot = '\0';
                printf("target_mmap libname=%s, target_prot=%d\n", libname, target_prot);
            }
        }
    }
#endif
    ...
    if (!(flags & MAP_FIXED)) {
        ...
        if (real_start < real_end) {
	        ...
	        if(fd != -1)
	        {
                struct stat st;
                fstat(fd, &st);
                abi_ulong file_size = HOST_PAGE_ALIGN(st.st_size);
                abi_ulong actu_size = real_end - real_start;

                if(file_size < actu_size)
                {
                    if (!(host_prot & PROT_WRITE)) {
                        mprotect (g2h_untagged(real_start), actu_size, host_prot | PROT_WRITE);}
                    p = pread(fd, g2h_untagged(real_start), actu_size, offset1);
                    if (!(host_prot & PROT_WRITE)) {
                        mprotect (g2h_untagged(real_start), actu_size, host_prot);}
                }else{
                    p = mmap(g2h_untagged(real_start), actu_size,
