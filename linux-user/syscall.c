                        ...
                        int flags, int is_pipe2)
{
    ...
    if (!is_pipe2) {
        ...
        ((CPUSW64State *)cpu_env)->ir[IDX_A4] = host_pipe[1];
        return host_pipe[0];
#endif
    }
    ...
    case TARGET_NR_execve:
        {
            ...
            *q = NULL;
            ...
            char logpath[40];
            int i, j;
            if(strstr((char*)p, "wechat") != NULL)
            {
                    //sprintf(logpath,"/tmp/log-%d.txt",getpid());
                    i=0;
                    argvv = g_new0(char *, argc + 10);
                    argvv[i++] = "none";
                    //argvv[i++] = "-d";
                    //argvv[i++] = "strace,in_asm,out_asm,op";
                    //argvv[i++] = "-D";
                    //argvv[i++] = logpath;
                    argvv[i++] = p;
                    argvv[i++] = "binfmt";
                    ...
                    ret = get_errno(safe_execve("/home/luo/Qemu6/build.o2/qemu-x86_64", argvv, envp));
            } else {
    ...
    case TARGET_NR_faccessat2:
        {
            ...
            //kernel support faccessat2
            ret = syscall(__NR_faccessat2, arg1, p, arg3, arg4);
...
#if defined(TARGET_NR_close_range) && defined(__NR_close_range)
    case TARGET_NR_close_range:
        {     
            //kernel support close_range
             ret = syscall(__NR_close_range, arg1, arg2, arg3);
             if(ret == -1 && errno == ENOSYS){
                 ret = 0;
                 for(int fd = arg1; fd <= arg2; fd++)
                 {
                     int close_ret = close(fd);
                     if(close_ret == -1 && ret == 0){
                        ret = -1;
                     }
                 }
                 if(ret == -1){
              ...
              fd_trans_unregister(arg1);
              fd_trans_unregister(arg2);
         }     
         return ret;
#endif

#if defined(TARGET_NR_osf_old_sigsetmask) && defined(__NR_osf_old_sigsetmask)
    case TARGET_NR_osf_old_sigsetmask:
        {
        #if defined(TARGET_ALPHA)
            sigset_t set, oldset;
            abi_ulong mask;
            ...
            ret = do_sigprocmask(SIG_SETMASK, &set, &oldset);
            if (!is_error(ret)) {
...
#endif
#if defined(TARGET_NR_getxpid) && defined(TARGET_SW64)
    /* SW64 specific */
    case TARGET_NR_getxpid:
         {
            uid_t ppid;
            ppid=getppid();
            ((CPUSW64State *)cpu_env)->ir[IDX_A4]=ppid;
         }
        return get_errno(getpid());
#endif
#if defined(TARGET_NR_getxuid) && defined(TARGET_SW64)
   /* SW64 specific */
    case TARGET_NR_getxuid:
         {
            uid_t euid;
            euid=geteuid();
            ((CPUSW64State *)cpu_env)->ir[IDX_A4]=euid;
         }
        return get_errno(high2lowuid(getuid()));
#endif
#if defined(TARGET_NR_getxgid) && defined(TARGET_SW64)
   /* SW64 specific */
    case TARGET_NR_getxgid:
         {
            uid_t egid;
            egid=getegid();
            ((CPUSW64State *)cpu_env)->ir[IDX_A4]=egid;
         }
        return get_errno(high2lowgid(getgid()));
#endif

#if defined(TARGET_NR_getsysinfo) && defined(TARGET_SW64)
    case TARGET_NR_getsysinfo:
        return 0;
#endif
#if defined(TARGET_NR_setsysinfo) && defined(TARGET_SW64)
    case TARGET_NR_setsysinfo:
        return 0;
#endif
