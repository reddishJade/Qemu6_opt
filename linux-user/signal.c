                                ...
                                void *puc)
{
    ...
    if ((host_signum == SIGSEGV || host_signum == SIGBUS)
        ...
        if((cpu && cpu->running) || in_code_gen_buffer(pc)){
            // 32 sc_regs: R0~R31
            env->regs[R_ESP] =  uc->uc_mcontext.sc_regs[8];
            env->regs[R_ESI] =  uc->uc_mcontext.sc_regs[10];
            env->regs[R_EDI] =  uc->uc_mcontext.sc_regs[11];
            env->regs[R_EAX] =  uc->uc_mcontext.sc_regs[12];
            env->regs[R_ECX] =  uc->uc_mcontext.sc_regs[13];
            env->regs[R_EDX] =  uc->uc_mcontext.sc_regs[14];
            env->regs[12] =  uc->uc_mcontext.sc_regs[6];
            env->regs[13] =  uc->uc_mcontext.sc_regs[7];
        }
#endif
                 ...
                 struct target_sigaction *oact)
{
    ...
    if (act) {
        ...
        __get_user(k->ka_restorer, &act->ka_restorer);
#endif
