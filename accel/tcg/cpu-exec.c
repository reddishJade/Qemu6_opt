...
static void align_clocks(SyncClocks *sc, CPUState *cpu)
{
    ...
    cpu_icount = cpu->icount_extra + cpu_neg(cpu)->icount_decr.u32.low;
...
static void init_delay_params(SyncClocks *sc, CPUState *cpu)
{
    ...
    sc->last_cpu_icount
        = cpu->icount_extra + cpu_neg(cpu)->icount_decr.u32.low;
...
#endif
#if defined(CONFIG_INDIRECT_HYPERCHAIN)
        /* TBs generated by pre_translate() bypass tb_find()'s post-generation
         * hooks.  Prepare their learned indirect slots here before direct
         * chaining can execute them without returning to the dispatcher. */
        prepare_hyperchain(next, cpu, cs_base, flags, cflags);
#endif
...
static void prepare_oracle_top1(TranslationBlock *tb, CPUState *cpu,
                                target_ulong cs_base, uint32_t flags,
                                uint32_t cflags)
{
    TranslationBlock *target;

    if (!tb->oracle_top1_pc || !tb->oracle_top1_patch_offset ||
        qemu_loglevel_mask(CPU_LOG_TB_NOCHAIN)) {
        return;
    }

    target = tb_lookup(cpu, tb->oracle_top1_pc, cs_base, flags, cflags);
    if (!target) {
        target = tb_gen_code(cpu, tb->oracle_top1_pc, cs_base, flags, cflags);
    }
    if (target) {
        qatomic_set(&cpu->tb_jmp_cache[
                        tb_jmp_cache_hash_func(tb->oracle_top1_pc)], target);
        patch_oracle_top1(tb, target);
    }
}
#endif

#if defined(CONFIG_INDIRECT_ORACLE_TOP2) && defined(__sw_64__)
static void prepare_oracle_top2(TranslationBlock *tb, CPUState *cpu,
                                target_ulong cs_base, uint32_t flags,
                                uint32_t cflags)
{
    TranslationBlock *target;
    ...
    for (int i = 0; i < 2; i++) {
        if (!tb->oracle_top2_pc[i] || !tb->oracle_top2_patch_offset[i]) {
            continue;
        }
        target = tb_lookup(cpu, tb->oracle_top2_pc[i], cs_base, flags, cflags);
        if (!target) {
            target = tb_gen_code(cpu, tb->oracle_top2_pc[i], cs_base,
                                 flags, cflags);
        }
        if (target) {
            qatomic_set(&cpu->tb_jmp_cache[
                            tb_jmp_cache_hash_func(tb->oracle_top2_pc[i])],
                        target);
            patch_oracle_top2(tb, target, i);
        }
    }
}
#endif
                                        ...
                                        int tb_exit, uint32_t cflags)
{
    ...
    if (tb == NULL) {
...
#endif
#if defined(CONFIG_INDIRECT_ORACLE_TOP1) && defined(__sw_64__)
        prepare_oracle_top1(tb, cpu, cs_base, flags, cflags);
#endif
#if defined(CONFIG_INDIRECT_ORACLE_TOP2) && defined(__sw_64__)
        ...
        if (tb_cflags(tb) & CF_PLT_STUB) {
             ...
             * tb_lookup()'s exact cflags match also makes this TB
             * invisible to normal lookups (CF_PLT_STUB != 0). */
            return tb;
        }
#endif
...
#if defined(CONFIG_INDIRECT_ORACLE_TOP1) && defined(__sw_64__)
    } else if (tb->oracle_top1_pc && tb->oracle_top1_patch_offset) {
        /* A TB may have been generated by another pre-translation path. */
        mmap_lock();
        prepare_oracle_top1(tb, cpu, cs_base, flags, cflags);
        mmap_unlock();
#endif
...
static inline bool cpu_handle_exception(CPUState *cpu, int *ret)
{
    if (cpu->exception_index < 0) {
        ...
        if (replay_has_exception()
            && cpu_neg(cpu)->icount_decr.u32.low + cpu->icount_extra == 0) {
                                        ...
                                        TranslationBlock **last_tb)
{
     ...
     */
    qatomic_mb_set(&cpu_neg(cpu)->icount_decr.u32.high, 0);
                                    ...
                                    TranslationBlock **last_tb, int *tb_exit)
{
    ...
    //the tb is the same as h2(next_tb) & ~TB_EXIT_MASK
    //tb_exit is the same as h2(next_tb&TB_EXIR_MASK)
    ...
    *last_tb = NULL;
    insns_left = qatomic_read(&cpu_neg(cpu)->icount_decr.u64);
    if (insns_left < 0) {
         ...
         * clear cpu->icount_decr.u64.high.
         */
        return;
    }
    ...
    cpu_neg(cpu)->icount_decr.u32.low = insns_left;
