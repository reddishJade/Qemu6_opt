...
uint64_t helper_fcvtws(CPUSW64State *env, uint32_t a)
{
    float32 fr = int32_to_float32(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return float32_to_s(fr);
}
...
uint64_t helper_fcvtwd(CPUSW64State *env, uint32_t a)
{
    float64 fr = int32_to_float64(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return (uint64_t)fr;
}

uint64_t helper_cmovuls(CPUSW64State *env, uint64_t a)
{
    float32 fr = uint64_to_float32(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return float32_to_s(fr);
}

uint64_t helper_cmovuws(CPUSW64State *env, uint32_t a)
{
    float32 fr = uint32_to_float32(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return float32_to_s(fr);
}

uint64_t helper_cmovuld(CPUSW64State *env, uint64_t a)
{
    float64 fr = uint64_to_float64(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return (uint64_t)fr;
}

uint64_t helper_cmovuwd(CPUSW64State *env, uint32_t a)
{
    float64 fr = uint32_to_float64(a, &FP_STATUS);
    env->error_code = soft_to_errcode_exc(env);
    return (uint64_t)fr;
}
...
static uint64_t do_cmovdlu(CPUSW64State *env, uint64_t a, uint64_t roundmode)
{
    uint64_t frac, ret = 0;
    uint32_t exp, exc = 0;
    int shift;
...
uint64_t helper_cmovdlu(CPUSW64State *env, uint64_t a, uint64_t roundmode)
{
    return do_cmovdlu(env, a, roundmode);
}

uint64_t helper_cmovdlu_dyn(CPUSW64State *env, uint64_t a)
{
    uint64_t roundmode = (uint64_t)(env->fpcr_round_mode);
    return do_cmovdlu(env, a, roundmode);
}
