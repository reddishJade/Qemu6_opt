...
#if defined(sw_to_sw)
static TCGv load_gfr(DisasContext *ctx, unsigned reg)
{
    if (likely(reg < 31)) {
        return ctx->fr[reg];
    } else {
    ...
    int ra, int rb, int32_t disp16, bool fp, bool clear)
{
    ...
    if (disp16 > 0) {
        ...
        tcg_gen_subi_i64(tmp, addr, (int64_t)(-disp16));
        addr = tmp;
    }else {
...
#ifdef CONFIG_TARGET_SW64_CPU_CORE4
static inline void gen_load_mem_a(                                                
    DisasContext *ctx, void (*tcg_gen_qemu_load)(TCGv t0, TCGv t1, int flags),
    int ra, int rb, int32_t disp12, bool fp, bool clear)
{
    TCGv tmp, addr, va;

    if (unlikely(ra == 31) || unlikely(rb == 31) || unlikely(ra == rb)) {
        return;
     }
    ...
    if (disp12 > 0) {
        tcg_gen_addi_i64(tmp, addr, (int64_t)disp12);
        addr = tmp;
    } else if (disp12 < 0) {
        tcg_gen_subi_i64(tmp, addr, (int64_t)(-disp12));
        addr = tmp;
    }else {
        tcg_gen_mov_i64(tmp, addr);
        addr = tmp;
    ...
    tcg_gen_mov_i64(load_gir(ctx, rb), addr);
    tcg_temp_free(tmp);
}
#endif

#if defined(sw_to_sw)
static inline void ldx(DisasContext *ctx, void (*tcg_gen_ldst)(TCGv t0, TCGv_ptr t1, tcg_target_long offset), int ra, int rb, int32_t disp16, bool fp)
{
    TCGv vb, va;
    ...
    va = (fp ? ctx->fr[ra] : ctx->ir[ra]);
    vb = load_gir(ctx, rb);
    tcg_gen_ldst(va, (TCGv_ptr)vb, disp16);
}

static inline void stx(DisasContext *ctx, void (*tcg_gen_ldst)(TCGv t0, TCGv_ptr t1, tcg_target_long offset), int ra, int rb, int32_t disp16, bool fp)
{
    TCGv vb, va;

    va = (fp ? load_gfr(ctx, ra) : load_gir(ctx, ra));
    vb = ctx->ir[rb];
    tcg_gen_ldst(va, (TCGv_ptr)vb, disp16);
}
#endif
...
#ifdef CONFIG_TARGET_SW64_CPU_CORE4
static inline void gen_store_mem_a(
    DisasContext *ctx, void (*tcg_gen_qemu_store)(TCGv t0, TCGv t1, int flags),
    int ra, int rb, int32_t disp12, bool fp, bool clear)
{
    TCGv tmp, addr, va;
                             ...
                             int32_t disp13, uint16_t fn)
{
    ...
    case 0x2d:
        /* SBT */
        tmp = tcg_temp_new();
        tmp1 = tcg_const_i64(1);
        tcg_gen_andi_i64(tmp, vb, 0x3f);
        tcg_gen_shl_i64(tmp, tmp1, tmp);        
    ...
    case 0x4c:
        ...
        tmp = tcg_temp_new();
        tmp1 = tcg_temp_new();
        tcg_gen_andi_i64(tmp, vb, 0x1fUL);        
        tcg_gen_shl_i64(tmp1, va, tmp);
        tcg_gen_ext32s_i64(vc, tmp1);
        tcg_temp_free(tmp);
        tcg_temp_free(tmp1);
        break;
    case 0x4d:
    ...
    case 0x4e:
        ...
        tmp1 = tcg_temp_new();
        tcg_gen_andi_i64(tmp, vb, 0x1fUL);        
        tcg_gen_ext32s_i64(tmp1, va); 
        tcg_gen_sar_i64(vc, tmp1, tmp);
        tcg_temp_free(tmp);
        tcg_temp_free(tmp1);
        break;
    case 0x4f:
        ...
        tcg_gen_extrl_i64_i32(tmpa, va);
        tcg_gen_extrl_i64_i32(tmpb, tmp);
   ...
   case 0x5b:
        /* REVBH */
        tcg_gen_bswap16_i64(vc, vb);
        break;
    case 0x5c:
        /* REVBW */
        tcg_gen_bswap32_i64(vc, vb);
        break;
    case 0x5d:
        /* REVBL */
...
#ifdef CONFIG_TARGET_SW64_CPU_CORE4
static void gen_cmovdl(DisasContext *ctx, int rb, int rc, uint64_t round_mode)
{
    TCGv tmp64;
    tmp64 = tcg_temp_new_i64();
    tcg_gen_movi_i64(tmp64, round_mode);
    gen_helper_fcvtdl(load_gir(ctx,rc), cpu_env, cpu_fr[rb], tmp64);   
    tcg_temp_free(tmp64);
    gen_fp_exc_raise(rc);
}

static void gen_cmovdlu(DisasContext *ctx, int rb, int rc, uint64_t round_mode)
{
    TCGv tmp64;
    tmp64 = tcg_temp_new_i64();
    tcg_gen_movi_i64(tmp64, round_mode);
    gen_helper_cmovdlu(load_gir(ctx,rc), cpu_env, cpu_fr[rb], tmp64);   
    tcg_temp_free(tmp64);
    gen_fp_exc_raise(rc);
}

static void gen_cmovdw(DisasContext *ctx, int rb, int rc, uint64_t round_mode)
{
    TCGv tmp64,tmp;
    tmp = tcg_temp_new_i64();
    tmp64 = tcg_temp_new_i64();
    ...
    tcg_gen_movi_i64(tmp64, round_mode);
    gen_helper_fcvtdl(tmp, cpu_env, cpu_fr[rb], tmp64);   
    tcg_gen_ext32s_i64(load_gir(ctx,rc), tmp);
...
static void gen_cmovdwu(DisasContext *ctx, int rb, int rc, uint64_t round_mode)
{
    TCGv tmp,tmp64;
    tmp = tcg_temp_new_i64();
    tmp64 = tcg_temp_new_i64();
    ...
    tcg_gen_movi_i64(tmp64, round_mode);
    gen_helper_cmovdlu(tmp, cpu_env, cpu_fr[rb], tmp64);   
    tcg_gen_ext32u_i64(load_gir(ctx,rc), tmp);
                             ...
                             uint8_t rb, uint8_t fn)
{
    ...
    case 0x48: 
        /* CMOVLS */
        gen_helper_fcvtls(cpu_fr[rc], cpu_env, load_gir(ctx, rb));
        break; 
    case 0x49:
        /* CMOVWS */
        tmp64 = tcg_temp_new();
        tcg_gen_andi_i64(tmp64, load_gir(ctx, rb), 0xffffffffu);
        gen_helper_fcvtws(cpu_fr[rc], cpu_env, tmp64);
        tcg_temp_free(tmp64);
        break; 
    case 0x4a: 
        /* CMOVLD */
        gen_helper_fcvtld(cpu_fr[rc], cpu_env, load_gir(ctx, rb));
        break; 
    case 0x4b: 
        /* CMOVWD */
        tmp64 = tcg_temp_new();
        tcg_gen_andi_i64(tmp64, load_gir(ctx, rb), 0xffffffffu);
        gen_helper_fcvtwd(cpu_fr[rc], cpu_env, tmp64);
        tcg_temp_free(tmp64);
        break;
    case 0x4c: 
        /* CMOVULS */
        gen_helper_cmovuls(cpu_fr[rc], cpu_env, load_gir(ctx, rb));
        break; 
    case 0x4d: 
        /* CMOVUWS */
        tmp64 = tcg_temp_new();
        tcg_gen_andi_i64(tmp64, load_gir(ctx, rb), 0xffffffffu);
        gen_helper_cmovuws(cpu_fr[rc], cpu_env, tmp64);
        tcg_temp_free(tmp64);
        break; 
    case 0x4e: 
        /* CMOVULD */
        gen_helper_cmovuld(cpu_fr[rc], cpu_env, load_gir(ctx, rb));
        break; 
    case 0x4f:
        /* CMOVUWD */
        tmp64 = tcg_temp_new();
        tcg_gen_andi_i64(tmp64, load_gir(ctx, rb), 0xffffffffu);
        gen_helper_cmovuwd(cpu_fr[rc], cpu_env, tmp64);
        tcg_temp_free(tmp64);
        break;
#endif 
    case 0x50:
    ...
    case 0x5a:
        /* FRIS */
        gen_helper_fcvtdl_dyn(cpu_fr[rc], cpu_env, cpu_fr[rb]);
        gen_helper_fcvtls(cpu_fr[rc], cpu_env, cpu_fr[rc]); 
        break;
    case 0x5b:
        /* FRIS_G */
        gen_fcvtdl(rb, rc, 0);
    ...
    case 0x60:
        /* FRID */
        gen_helper_fcvtdl_dyn(cpu_fr[rc], cpu_env, cpu_fr[rb]);
        gen_helper_fcvtld(cpu_fr[rc], cpu_env, cpu_fr[rc]); 
        break;
    case 0x61:
        /* FRID_G */
        gen_fcvtdl(rb, rc, 0);
    ...
    case 0x72:
        /* CMOVDL */
        gen_helper_fcvtdl_dyn(load_gir(ctx, rc), cpu_env, cpu_fr[rb]);
        break;
    case 0x74:
        /* CMOVDL_G */
        gen_cmovdl(ctx, rb, rc, 0);
        break;
    case 0x7a:
        /* CMOVDL_P */
        gen_cmovdl(ctx, rb, rc, 2);
        break;
    case 0x7c:
        /* CMOVDL_Z */
        gen_cmovdl(ctx, rb, rc, 3);
        break;
    case 0x80:
        /* CMOVDL_N */
        gen_cmovdl(ctx, rb, rc, 1);
        break;
    case 0x81:
        /* CMOVDLU */
        gen_helper_cmovdlu_dyn(load_gir(ctx, rc), cpu_env, cpu_fr[rb]);
        break;
    case 0x82:
        /* CMOVDLU_G */
        gen_cmovdlu(ctx, rb, rc, 0);
        break;
    case 0x83:
        /* CMOVDLU_P */
        gen_cmovdlu(ctx, rb, rc, 2);
        break;
    case 0x84:
        /* CMOVDLU_Z */
        gen_cmovdlu(ctx, rb, rc, 3);
        break;
    case 0x85:
        /* CMOVDLU_N */
        gen_cmovdlu(ctx, rb, rc, 1);
        break;
    case 0x86:
        /* CMOVDWU */
        tmp64 = tcg_temp_new_i64();
        gen_helper_cmovdlu_dyn(tmp64, cpu_env, cpu_fr[rb]);   
        tcg_gen_ext32u_i64(load_gir(ctx,rc), tmp64); 
        tcg_temp_free(tmp64);
        break;
    case 0x87:
        /* CMOVDWU_G */
        gen_cmovdwu(ctx, rb, rc, 0);
        break;
    case 0x88:
        /* CMOVDWU_P */
        gen_cmovdwu(ctx, rb, rc, 2);
        break;
    case 0x89:
        /* CMOVDWU_Z */
        gen_cmovdwu(ctx, rb, rc, 3);
        break;
    case 0x8a:
        /* CMOVDWU_N */
        gen_cmovdwu(ctx, rb, rc, 1);
        break;
    case 0x8b:
        /* CMOVDW */
        tmp64 = tcg_temp_new_i64();
        gen_helper_fcvtdl_dyn(tmp64, cpu_env, cpu_fr[rb]);   
        tcg_gen_ext32s_i64(load_gir(ctx,rc), tmp64); 
        tcg_temp_free(tmp64);
        break;
    case 0x8c:
        /* CMOVDW_G */
        gen_cmovdw(ctx, rb, rc, 0);
        break;
    case 0x8d:
        /* CMOVDW_P */
        gen_cmovdw(ctx, rb, rc, 2);
        break;
    case 0x8e:
        /* CMOVDW_Z */
        gen_cmovdw(ctx, rb, rc, 3);
        break;
    case 0x8f:
        /* CMOVDW_N */
        gen_cmovdw(ctx, rb, rc, 1);
        break;
#endif
    default:
                                           ...
                                           MemOp op)
{
    ...
    if(disp16 > 0) {
        ...
        tcg_gen_subi_i64(addr, load_gir(ctx, rb), -disp16);
    }
...
#ifndef CONFIG_TARGET_SW64_CPU_CORE4
    ...
    #ifdef CONFIG_TARGET_SW64_CPU_CORE4
        tcg_gen_setcond_i64(TCG_COND_EQ, load_gir(ctx, ra), val, cpu_lock_value);
    #else
...
#ifdef CONFIG_TARGET_SW64_CPU_CORE4
    tcg_gen_movi_i64(load_gir(ctx, ra), 0);
#else
                           ...
                           bool bype)
{
    ...
    if(disp12 > 0) {
        ...
        tcg_gen_subi_i64(tmp, addr, -disp12);
    }
    addr = tmp;
...
DisasJumpType translate_one(DisasContextBase *dcbase, uint32_t insn,
                                   CPUState *cpu)
{
    int32_t disp5, disp8, disp12, disp13, disp16, disp21;
    ...
    case 0x06:
        ...
        case 0x1B:
            ...
            case 0x26:
                ...
                tmp64_0 = tcg_temp_new();
                vd = tcg_const_i64(rc);
                tcg_gen_andi_i64(tmp64_0, cpu_fr[rd], 0x1ful);
                tcg_gen_shri_i64(tmp64_0, tmp64, 2);
            ...
            case 0x28:
                ...
                tmp64_0 = tcg_temp_new();
                vd = tcg_const_i64(rc);
                tcg_gen_andi_i64(tmp64_0, cpu_fr[rd], 0xful);
                tcg_gen_shri_i64(tmp64_0, tmp64_0, 2);
            ...
            case 0x29:
                ...
                vd = tcg_const_i64(rc);
                tcg_gen_andi_i64(tmp64_0, cpu_fr[rd], 0x1ful);
                tcg_gen_shri_i64(tmp64_0, tmp64_0, 3);
                gen_helper_vcond(cpu_env, tmp64, vd, tmp64_0);
                tcg_temp_free(tmp64_0);
                tcg_temp_free(tmp64);
                tcg_temp_free(vd);
        ...
        case 0x1E:
            ...
            case 0x0:  
                /* LDBU_A */
                gen_load_mem_a(ctx, &tcg_gen_qemu_ld8u, ra, rb, disp12, 0, 0);
                break;
            case 0x1:
                /* LDHU_A */
                gen_load_mem_a(ctx, &tcg_gen_qemu_ld16u, ra, rb, disp12, 0, 0);
                break;  
            case 0x2:
                /* LDW_A */
                gen_load_mem_a(ctx, &tcg_gen_qemu_ld32s, ra, rb, disp12, 0, 0);
                break;  
            case 0x3:
                /* LDL_A */
                gen_load_mem_a(ctx, &tcg_gen_qemu_ld64, ra, rb, disp12, 0, 0);
                break;  
            case 0x4:
                /* FLDS_A */
                gen_load_mem_a(ctx, &gen_qemu_flds, ra, rb, disp12, 1, 0);
                break;  
            case 0x5:
                /* FLDD_A */
                gen_load_mem_a(ctx, &tcg_gen_qemu_ld64, ra, rb, disp12, 1, 0);
                break;
            case 0x6:
                /* STB_A */
                gen_store_mem_a(ctx, &tcg_gen_qemu_st8, ra, rb, disp12, 0, 0);
                break;  
            case 0x7:
                /* STH_A */
                gen_store_mem_a(ctx, &tcg_gen_qemu_st16, ra, rb, disp12, 0, 0);
                break;  
            case 0x8:
                /* STW_A */
                gen_store_mem_a(ctx, &tcg_gen_qemu_st32, ra, rb, disp12, 0, 0);
                break;  
            case 0x9:
                /* STL_A */
                gen_store_mem_a(ctx, &tcg_gen_qemu_st64, ra, rb, disp12, 0, 0);
                break;  
            case 0xA:
                /* FSTS_A */
                gen_store_mem_a(ctx, &gen_qemu_fsts, ra, rb, disp12, 1, 0);
                break;  
            case 0xB:
                /* FSTD_A */
                gen_store_mem_a(ctx, &tcg_gen_qemu_st64, ra, rb, disp12, 1, 0);
                break;  
            default:
                printf("ILLEGAL BELOW OPC[%x] func[%08x]\n", opc, fn4);
                ret = gen_invalid(ctx);
        ...
        case 0x20:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_ld8u_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x21:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_ld16u_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x22:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_ld32s_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x23:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_ld_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x24:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_ldl_u, ra, rb, disp16, 0);
#else
        ...
        case 0x26:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            ldx(ctx, &tcg_gen_flds, ra, rb, disp16, 1);
#else
        ...
        case 0x27:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            //ldx(ctx, &tcg_gen_ld_i64, ra, rb, disp16, 1);
            ldx(ctx, &tcg_gen_fldd, ra, rb, disp16, 1);
#else
        ...
        case 0x28:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_st8_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x29:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_st16_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x2a:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_st32_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x2b:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_st_i64, ra, rb, disp16, 0);
#else
        ...
        case 0x2c:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_stl_u, ra, rb, disp16, 0);
#else
        ...
        case 0x2e:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            stx(ctx, &tcg_gen_fsts, ra, rb, disp16, 1);
#else
        ...
        case 0x2f:
            ...
            disp16 = sextract32(insn, 0, 16);
#if defined(sw_to_sw)
            //stx(ctx, &tcg_gen_st_i64, ra, rb, disp16, 1);
            stx(ctx, &tcg_gen_fstd, ra, rb, disp16, 1);
#else
        ...
        case 0x3f:
            ...
            if (rb == 31) {
                ...
                vb = load_gir(ctx, rb);
                if (disp16 == 0) {
                    tcg_gen_mov_i64(va, vb);
#if defined(sw_to_sw)
                } else {
                    tcg_gen_ldih(va, vb, tcg_constant_i64((int64_t)disp16));
#else
                } else if (disp16 > 0) {
                    tcg_gen_addi_i64(va, vb, (int64_t)disp16);
                } else {
                    tcg_gen_subi_i64(va, vb, -(int64_t)disp16);
#endif
                }
            }
            break;
        case 0x3e:
            ...
            if (rb == 31) {
                ...
                if (disp16 == 0) {
                    ...
                    tcg_gen_ldi(va, vb, tcg_constant_i64((int64_t)disp16));
#else
                } else if (disp16 > 0) {
                    tcg_gen_addi_i64(va, vb, (int64_t)disp16);
                } else {
